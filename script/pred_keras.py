...
def predict2(model, input_file, out_dir, scale = 2.0, ch = 1):
    ...
    lr_img = lr_img.resize(img.size, Image.BICUBIC)
    ...
    h,w,c = yuv.shape
    y_hr = y_hr[0:h,0:w]
    #Image.fromarray(y_hr).convert('RGB').save('%s/%s_hr%s' % (out_dir, filename, ext))
...
def predict_hr_img(model, lr_img, hr_img_size, scale, out_dir):
    ...
    for y in range(0, h, patch_size):
        for x in range(0, w, patch_size):
            ...
            hr_img[y:y+patch_size,x:x+patch_size] = res
